...
- [ ] D2-2.0.1 Extract finalized problem statement from D1 into D2 intro.
- [ ] D2-2.0.2 Extract finalized hypothesis targets (sub-second ETTR, <5% overhead) into D2 goals.
- [ ] D2-2.0.3 Convert D1 literature synthesis into a concise D2 related-work baseline section.
- [ ] D2-2.0.4 Build requirement traceability matrix mapping each required D2 section to subsections.
- [ ] D2-2.0.5 Define explicit assumptions list (hardware, network, framework versions).
- [ ] D2-2.0.6 Define explicit non-goals list to prevent scope creep.

### D2-2.1 System Architecture Diagram (Required)
- [ ] D2-2.1.1 Confirm node inventory (4 nodes, GPU model, CPU RAM, NIC details).
- [ ] D2-2.1.2 Define rank-to-device mapping policy (global ranks 0 to 3).
- [ ] D2-2.1.3 Draw physical topology (instances, network boundaries, control plane).
- [ ] D2-2.1.4 Draw logical topology (process groups, rank ring order).
- [ ] D2-2.1.5 Overlay Ring-All-Reduce data flow arrows and direction.
- [ ] D2-2.1.6 Overlay MTGS control flow channels (prepare, vote, commit, abort).
- [ ] D2-2.1.7 Mark failure observation points in the diagram (rank crash, timeout, link interruption).
- [ ] D2-2.1.8 Add legend, notation, and message labels.
- [ ] D2-2.1.9 Export architecture artifact in editable and static format.
- [ ] D2-2.1.10 Perform peer review for ambiguity and missing labels.

### D2-2.2 Memory Hierarchy Design (Required)
- [ ] D2-2.2.1 Define all state categories (active params, gradients, optimizer state, shadow state).
- [ ] D2-2.2.2 Map each state category to location (GPU VRAM or CPU RAM).
- [ ] D2-2.2.3 Define state life cycle per micro-batch (allocate, copy, consume, release).
- [ ] D2-2.2.4 Specify pinned-memory usage policy and rationale.
- [ ] D2-2.2.5 Specify async copy stream ordering and synchronization points.
- [ ] D2-2.2.6 Quantify expected RAM amplification factor from shadow states.
- [ ] D2-2.2.7 Document memory pressure thresholds and fallback behavior.
- [ ] D2-2.2.8 Add hierarchy diagram with labeled bandwidth/latency assumptions.

### D2-2.3 Communication Model (Required)
- [ ] D2-2.3.1 Document baseline PyTorch gradient synchronization sequence.
- [ ] D2-2.3.2 Define MTGS transaction boundaries per micro-batch.
- [ ] D2-2.3.3 Define prepare message schema and trigger conditions.
- [ ] D2-2.3.4 Define vote message schema and quorum rules.
- [ ] D2-2.3.5 Define commit and abort message schema and state transitions.
- [ ] D2-2.3.6 Draw sequence diagram combining gradient sync and 2PC control.
- [ ] D2-2.3.7 Identify communication critical path and additional round trips.
- [ ] D2-2.3.8 List timeout values and retry behavior for each control message.

### D2-2.4 Payload Complexity Analysis (Required)
- [ ] D2-2.4.1 List all tensors participating in gradient synchronization.
- [ ] D2-2.4.2 Derive closed-form payload equation in bytes per micro-batch.
- [ ] D2-2.4.3 Convert payload equation to MB using consistent base (MiB or MB).
- [ ] D2-2.4.4 Derive additional MTGS control-plane payload equation.
- [ ] D2-2.4.5 Compare baseline and MTGS total bytes per step.
- [ ] D2-2.4.6 State complexity class with respect to rank count and parameter count.
- [ ] D2-2.4.7 Validate equations with at least one worked numerical example.

#### D2-2.4.1 Coding: Payload Profiler Script
- [ ] D2-2.4.1.1 Create script skeleton for DistilBERT gradient payload profiling.
- [ ] D2-2.4.1.2 Load model configuration and parameter tensors.
- [ ] D2-2.4.1.3 Compute total trainable parameter count.
- [ ] D2-2.4.1.4 Compute payload bytes for fp32, fp16, and bf16 assumptions.
- [ ] D2-2.4.1.5 Emit per-layer and aggregate payload summary.
- [ ] D2-2.4.1.6 Log output in machine-readable format (CSV or JSON).
- [ ] D2-2.4.1.7 Add argument flags for model name and precision.
- [ ] D2-2.4.1.8 Verify numbers against manual calculation sample.

### D2-2.5 Consistency Model (Required)
- [ ] D2-2.5.1 Define consistency target (equivalence to fault-free BSP step outcome).
- [ ] D2-2.5.2 Define formal state variables for pre-commit and post-commit views.
- [ ] D2-2.5.3 Define transaction invariants that must always hold.
- [ ] D2-2.5.4 Prove no partial gradient application on abort path.
- [ ] D2-2.5.5 Prove commit path produces globally aligned model state across ranks.
- [ ] D2-2.5.6 Provide counterexample discussion and explain why protocol blocks it.
- [ ] D2-2.5.7 Document assumptions needed for proof validity.
- [ ] D2-2.5.8 Add short proof sketch and expanded appendix proof.

### D2-2.6 Failure Model (Required)
- [ ] D2-2.6.1 Enumerate failure types considered (SIGTERM, SIGKILL, timeout, OOM).
- [ ] D2-2.6.2 Explicitly mark fail-stop assumptions and excluded Byzantine behavior.
- [ ] D2-2.6.3 Model mid-All-Reduce crash behavior on communicator integrity.
- [ ] D2-2.6.4 Model failure detection mechanism and latency budget.
- [ ] D2-2.6.5 Define abort propagation logic and safe stop condition.
- [ ] D2-2.6.6 Define rollback initiation and completion condition.
- [ ] D2-2.6.7 Define rejoin policy (if supported) or restart policy (if not supported).
- [ ] D2-2.6.8 Provide failure timeline diagram from fault to resumed training.

### D2-2.7 Performance Modeling (Required)
- [ ] D2-2.7.1 Define baseline throughput model with decomposed cost terms.
- [ ] D2-2.7.2 Introduce MTGS overhead terms (control messages, copies, rollback checks).
- [ ] D2-2.7.3 Apply Amdahl model using control overhead as serial fraction.
- [ ] D2-2.7.4 Apply Gustafson model for scaled workload perspective.
- [ ] D2-2.7.5 Add message complexity estimate per step and per epoch.
- [ ] D2-2.7.6 Produce sensitivity table for overhead at multiple failure rates.
- [ ] D2-2.7.7 Identify parameter regimes where MTGS is net beneficial.
- [ ] D2-2.7.8 Cross-check model variables against planned measured metrics.

#### D2-2.7.1 Coding: Amdahl and Gustafson Plotter
- [ ] D2-2.7.1.1 Implement script inputs for serial fraction, node count, and failure rate.
- [ ] D2-2.7.1.2 Generate Amdahl speedup curves for 1 to 8 nodes.
- [ ] D2-2.7.1.3 Generate Gustafson scaled speedup curves for matching inputs.
- [ ] D2-2.7.1.4 Overlay baseline and MTGS predicted curves.
- [ ] D2-2.7.1.5 Export publication-ready plots and CSV source values.
- [ ] D2-2.7.1.6 Add script usage examples in comments or README snippet.

### D2-2.8 Scalability Assumptions (Required)
- [ ] D2-2.8.1 Define weak-scaling assumption set.
- [ ] D2-2.8.2 Define strong-scaling assumption set.
- [ ] D2-2.8.3 Define network scaling assumptions and bottleneck thresholds.
- [ ] D2-2.8.4 Define CPU RAM scaling formula for shadow states.
- [ ] D2-2.8.5 Define GPU memory headroom requirement per rank.
- [ ] D2-2.8.6 State expected scaling limit and break-even conditions.

#### D2-2.8.1 Coding: Shadow State RAM Estimator
- [ ] D2-2.8.1.1 Implement formula-based estimator for 1 to 4 nodes.
- [ ] D2-2.8.1.2 Add inputs for precision mode and optimizer state multiplier.
- [ ] D2-2.8.1.3 Emit table with per-node and cluster-wide RAM requirements.
- [ ] D2-2.8.1.4 Include safety margin recommendations in output.
- [ ] D2-2.8.1.5 Validate estimator against one synthetic tensor-size case.

### D2-2.9 Implementation Plan (Required)
- [ ] D2-2.9.1 Break implementation into milestones (baseline, MTGS hook, rollback, evaluation).
- [ ] D2-2.9.2 Define deliverables for each milestone (scripts, logs, plots, report sections).
- [ ] D2-2.9.3 Assign team owners for each milestone and backup owner.
- [ ] D2-2.9.4 Define milestone entry and exit criteria.
- [ ] D2-2.9.5 Define integration points and branch strategy.
- [ ] D2-2.9.6 Define verification checkpoints per milestone.
- [ ] D2-2.9.7 Map milestones to submission timeline with risk buffer.

### D2-2.10 Risk Analysis (Required)
- [ ] D2-2.10.1 Create risk register with probability and impact score.
- [ ] D2-2.10.2 Add NCCL timeout risk and mitigation actions.
- [ ] D2-2.10.3 Add CPU RAM exhaustion risk and mitigation actions.
- [ ] D2-2.10.4 Add unstable spot instance churn risk and mitigation actions.
- [ ] D2-2.10.5 Add reproducibility drift risk and mitigation actions.
- [ ] D2-2.10.6 Add schedule slippage risk and mitigation actions.
- [ ] D2-2.10.7 Define risk trigger signals and response owner.
- [ ] D2-2.10.8 Define contingency plan for unavailable multi-node cloud time.

### D2-2.11 Academic Integrity and Citation Control
- [ ] D2-2.11.1 Build citation ledger for all reused ideas and figures.
- [ ] D2-2.11.2 Verify every comparative claim has a source.
- [ ] D2-2.11.3 Mark all external code snippets and record modifications.
- [ ] D2-2.11.4 Ensure no uncited text reuse from external materials.
- [ ] D2-2.11.5 Standardize citation style across report.

### D2-2.12 Deliverable 2 Assembly and Quality Gate
- [ ] D2-2.12.1 Assemble all required sections into one coherent design document.
- [ ] D2-2.12.2 Verify required-section checklist is 100 percent complete.
- [ ] D2-2.12.3 Perform internal review focused on C-5 analytical depth.
- [ ] D2-2.12.4 Revise unclear assumptions, missing equations, and unsupported claims.
- [ ] D2-2.12.5 Run final plagiarism and citation audit.
- [ ] D2-2.12.6 Mark D2 sign-off gate complete.
